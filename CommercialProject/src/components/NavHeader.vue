    ...
    methods: {
      checkLogin () {
        axios.get('/users/checkLogin').then((response) => {
          ...
          if (res.status === '0') {  // 当前登录的话，就保存用户名字
      ...
      logIn () {
        ...
        axios.post('/users/login', {
          ...
          let res = response.data  // 数据都在.data中
      ...
      getCartCount () {
        axios.get('/users/getCartCount').then((response) => {
          let res = response.data
          this.$store.coomit('updateCartCount', res.result)
        })
      }
    }
  }
