      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item, index) in addressListFilter" v-bind:class="{'check': checkIndex === index}" @click="checkIndex = index; selectAddress = item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
                  ...
                  <div class="addr-opration addr-set-default">
                    <a href="#" v-if="!item.isDefault" @click="setDefault(item.addressId)" class="addr-set-default-btn"><i>Set default</i></a>
                  </div>
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" v-bind:to="{path:'orderConfirm', query:{'addressId': selectAddress}}">Next</router-link>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      addressListFilter () {
        return this.addressList.slice(0, this.limit)  // 对addressList数组进行截取，并返回全新的数组，对原来的数组没有影响
      }
    },
    methods: {
      init () {
