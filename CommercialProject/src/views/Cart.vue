              ...
              <li v-for="item in cartList">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="#" @click="editCart('checked', item)" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked =='1'}"  >
  ...
  export default {
    ...
    data () {
      return {
        cartList: [],
        modalConfirm: false,
        productId: '' // 将商品 id 全局的缓存起来，
      }
    },
    mounted () {
      this.init()  // 组件加载完成之后，就会加载用户购物车数据
    },
    filters: {
//      currency: function (val) {  // 过滤器的值是一个函数，这个函数对接收的参数进行处理，并返回全新的值
//        return 'val'
//      }
      // currency: currency  // 由于currency就是一个函数，所以可以直接写
    },
    computed: {
      checkedCount () {
        let i = 0
        this.cartList.forEach((item) => {  // 对购物车的选中状态进行遍历
          if (item.checked === '1') i++
        })
        return i
      },
      checkAllFlag () {  // 计算属性中定义了 checkAllFlag, 在data中就不需要定义checkAllFlag
        return this.checkedCount === this.cartList.length // 当选中的数量等于购物车的数量，就代表全选中
      },
      totalPrice () {
        var money = 0
        this.cartList.forEach((item) => {  // 对购物车的选中状态进行遍历
    ...
    methods: {
      toggleCheckAll () {
        let flag = !this.checkAllFlag   // this.checkAllFlag = !this.checkAllFlag  // 由于checkAllFlag 进行了实时计算属性，这样赋值会失败
        this.cartList.forEach((item) => {
          item.checked = flag  // 将这里修改为 flag 就可以（原来写法：// item.checked = this.checkAllFlag）
        })
      ...
      delCartConfirm (productId) {
        this.productId = productId  // 将 productId 全局保存,这样在删除的时候，才可以作为参数传入
      ...
      delCart () {
        axios.post('/users/cartDel', {productId: this.productId}).then((response) => {
          let res = response.data
          if (res.status === '0') {
            this.modalConfirm = false // 删除成功后，将模态框关掉
            this.init()  // 数据重新渲染
          }
        })
      },
      ...
      checkOut () {
        if (this.checkedCount > 0) { // 只有选择了商品，才能进行跳转到地址页面
          this.$router.push({
            path: '/address'
          })
        }
